...
class SearchService implements SearchServiceInterface {
  ...
  List<Product>? sortFoodSearchList(
      ...
      int sortIndex,
      int priceSortIndex,
      int orderType // New parameter: 0 for instantOrder, 1 for scheduleOrders
      ) {
    List<Product>? searchProductList = [];
    searchProductList.addAll(allProductList!);
    ...
    // New filter for order type
    if (orderType == 0) { // Instant order products only
      searchProductList.removeWhere((product) => !product.instantorder!);
    } else if (orderType == 1) { // Schedule order products only
      searchProductList.removeWhere((product) => !product.scheduleorders!);
    }
    ...
    if (sortIndex != -1) {
      if (sortIndex == 0) {
        ...
        searchProductList.sort((a, b) => a.name!.toLowerCase().compareTo(b.name!.toLowerCase()));
        searchProductList = searchProductList.reversed.toList();
      }
    }
