...
class CartRepository implements CartRepositoryInterface<OnlineCart> {
  ...
  Future<List<OnlineCartModel>> get(String? id,) async {
    List<OnlineCartModel> onlineCartList = [];
    Response response = await apiClient.getData('${AppConstants.getCartListUri}${id != null ? '?guest_id=$id' : ''}?day=${Get.find<CartController>().day != null ? Get.find<CartController>().day :""}&time=${Get.find<CartController>().time !=null ? Get.find<CartController>().time :""}'); //?day=${Get.find<CartController>().day}&time=${Get.find<CartController>().time}
  log("data from cart list ${response.body.toString()} ");
    if(response.statusCode == 200) {
      onlineCartList = [];
      response.body.forEach((cart) => onlineCartList.add(OnlineCartModel.fromJson(cart)));
    }
