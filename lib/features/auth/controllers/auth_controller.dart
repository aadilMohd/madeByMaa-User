...
import '../domain/models/upload_photo_body.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> login1(String? phone) async {

    _isLoading = true;
    update();
    ResponseModel responseModel =
        await authServiceInterface.login1(phone: phone);
 ...
 RxBool isLoadingotp = false.obs;
  Future<ResponseModel> loginOtp({String? phone, String? otp}) async {
    isLoadingotp.value = true;
    update();
    ResponseModel responseModel =
        await authServiceInterface.loginOtp(otp: otp, phone: phone);
    isLoadingotp.value = false;

    status = responseModel.message;
    print("-=-=-ddd $status ${responseModel}");
    print("-=-=-ddd  ${responseModel.message}");
  ...
  Future<ResponseModel> registration1(SignUpBodyModel signUpModel) async {
    _isLoading = true;
    update();
    ResponseModel responseModel = await authServiceInterface.registration1(
      signUpModel,
    );
    _isLoading = false;
  ...
  Future<ResponseModel> uploadPhoto(UploadPhotoBody uploadPhotoBody) async {
    _guestLoading = true;
    update();
    ResponseModel responseModel = await authServiceInterface.uploadPhoto(uploadPhotoBody);
    _guestLoading = false;
    update();
    return responseModel;
  }
