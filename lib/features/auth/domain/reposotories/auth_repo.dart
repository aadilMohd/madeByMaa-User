...
class AuthRepo implements AuthRepoInterface<SignUpBodyModel> {
  ...
  @override
  Future<Response> registration1(SignUpBodyModel signUpModel,) async {

    String num = signUpModel.phone!.startsWith("+91")?signUpModel.phone!:"+91${signUpModel.phone!}";
    Map<String, String> body = {};
    body.addAll(<String, String>{
      'f_name': signUpModel.fName!, 'l_name': signUpModel.lName!, 'phone':num,'email': signUpModel.email!,  'ref_code': signUpModel.refCode!,'image': "null",
    });

    return await apiClient.postData(AppConstants.updateProfileUri, body,handleError: true);
    // return await apiClient.postData(AppConstants.registerUri, signUpModel.toJson(), handleError: false);
  }
  ...
  Future<Response> login1({String? phone}) async {
    // String guestId = getGuestId();
    Map<String, String> data = {
      "phone": phone!,
    };

    return await apiClient.postData(AppConstants.requestOtp, data, handleError: false);

  }
  Future<Response> loginOtp({String? phone,String?otp}) async {



    return await apiClient.postData(AppConstants.loginOtp,{"phone":phone,"otp":otp});
  ...
  @override
  Future<Response> uploadPhoto(UploadPhotoBody uploadPhotoBody) async {
    List<MultipartBody> photos = [];
    if (uploadPhotoBody.photo != null) {
      photos.add(MultipartBody(
        'photo',
        uploadPhotoBody.photo!,
      ));
    }

    return await apiClient.postMultipartData(
        AppConstants.photoUpload,
        {
          "user_id": uploadPhotoBody.userId!,
