...
import 'package:stackfood_multivendor/features/checkout/domain/models/avaliable_times_body.dart';
...
class CheckoutController extends GetxController implements GetxService {
  ...
  bool? checkCancelOrder = false;
  bool? checkScheduleOrder = false;
  ...
  String? todayDay;
  String? tomorrowDay;
  Future<Response> getAvailableTimes(int id) async {
    Response response = await checkoutServiceInterface.getAvailableTimes(id);
    if (response.statusCode == 200) {
      if (response.body.containsKey("available_times")) {
        availableTimes =
            AvailableTimes.fromJson(response.body["available_times"]);
        isAvailableTimeNull = false;
        if (kDebugMode) {
  ...
  List<Slot>? list = [];
  void assignData(String? selectedDate) {
    DateTime today = DateTime.now();
    DateTime tomorrow = today.add(const Duration(days: 1));
    todayDay = DateFormat('EEEE').format(today);
    tomorrowDay = DateFormat('EEEE').format(tomorrow);
    String? dayName;
    if (selectedDate != null && selectedDateSlot == 2) {
      DateTime date = DateTime.parse(selectedDate);

      // Use DateFormat to get the full weekday name (e.g., "Sunday")
      dayName = DateFormat('EEEE').format(date);
    }
    if (availableTimes != null) {
      if (_selectedDateSlot == 0) {
        list = getSlotsForDay(todayDay!);
      } else if (_selectedDateSlot == 1) {
        list = getSlotsForDay(tomorrowDay!);
      } else if (_selectedDateSlot == 2) {
        list = getSlotsForDay(dayName!);
      }
    }
    //list= availableTimes!.tuesday!.slots;
    //print("++++++++$_selectedDateSlot");
    print("++++++++${jsonEncode(list)}");
    print("++++++++${todayDay.toString().toLowerCase()}");
    print("++++++++${tomorrowDay.toString().toLowerCase()}");
  }

  List<Slot>? getSlotsForDay(String day) {
    switch (day.toLowerCase()) {
      case 'sunday':
        return availableTimes!.sunday?.slots;
      case 'monday':
        return availableTimes!.monday?.slots;
      case 'tuesday':
        return availableTimes!.tuesday!.slots;
      case 'wednesday':
        return availableTimes!.wednesday?.slots;
      case 'thursday':
        return availableTimes!.thursday?.slots;
      case 'friday':
        return availableTimes!.friday?.slots;
      case 'saturday':
        return availableTimes!.saturday?.slots;
      default:
        print("Invalid day.");
        return [];
    }
  ...
  Future<void> initCheckoutData(int? restaurantID) async {
    ...
    clearPrevData();
    print("*************1 ${restaurantID}");
    await Get.find<RestaurantController>()
        .getRestaurantDetails(Restaurant(id: restaurantID));
    print("************* 2 ${restaurantID}");
    initializeTimeSlot(Get.find<RestaurantController>().restaurant!);
    print(
        "************* 3 ${jsonEncode(Get.find<RestaurantController>().restaurant!)}");
  ...
  void setPreferenceTimeForView(String time, bool instanceOrder,
    ...
    print("fgfg $_preferableTime");
    if (isUpdate) {
      update();
    }
  }

  String setPreferenceTimeForView1(String time, bool instanceOrder,
      {bool isUpdate = false}) {
    String n =
        checkoutServiceInterface.setPreferenceTimeForView(time, instanceOrder);
    if (isUpdate) {
  ...
  void setCustomDate(DateTime? date, bool instanceOrder,
    ...
    log("customdateee ${_selectedCustomDate} ${_selectedTimeSlot}");
    if (canUpdate) {
      update();
    }
  }
