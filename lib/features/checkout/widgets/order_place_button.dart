...
class OrderPlaceButton extends StatefulWidget {
  ...
  const OrderPlaceButton(
      ...
      required this.guestEmailNode, this.isCheckOut=false,
...
class _OrderPlaceButtonState extends State<OrderPlaceButton> {
     ...
     width: Dimensions.webMaxWidth,
      ...
      child: SafeArea(
        child: CustomButtonWidget(
            ...
            (widget.checkoutController.checkCancelOrder!=true||widget.checkoutController.checkScheduleOrder!=true)?Colors.grey: const Color(0xff090018),
            buttonText: widget.checkoutController.isPartialPay
                ? 'place_order'.tr
                : 'confirm_order'.tr,
            radius: Dimensions.radiusDefault,
            isLoading: widget.checkoutController.isLoading,
            onPressed:(widget.checkoutController.checkCancelOrder==false||widget.checkoutController.checkScheduleOrder==false)?null: () {
              ...
              if (widget.checkoutController.isDmTipSave &&
                ...
                Get.find<AuthController>()
                    .saveDmTipIndex(widget.checkoutController.selectedTips.toString());
              }
              if (!widget.checkoutController.isDmTipSave) {
              ...
              if (_showsWarningMessage(
                ...
                log("wwwdataorderbody ${jsonEncode(placeOrderBody)}");
                if (widget.checkoutController.paymentMethodIndex == 3) {
                  Get.toNamed(RouteHelper.getOfflinePaymentScreen(
                    placeOrderBody: placeOrderBody,
                    zoneId: widget.checkoutController.restaurant!.zoneId!,
                    total: widget.total,
                    maxCodOrderAmount: widget.maxCodOrderAmount,
                    fromCart: widget.fromCart,
                    isCodActive: widget.isCashOnDeliveryActive,
                    pricingView: PricingViewModel(
                      subTotal: widget.subTotal,
                      subscriptionQty: widget.subscriptionQty,
                      discount: widget.discount!,
                      taxIncluded: widget.taxIncluded,
                      tax: widget.tax,
  ...
  DateTime _processScheduleStartDate() {
    ...
    log("wwwdataorderbody ${widget.checkoutController.selectedDateSlot} ${widget.checkoutController.selectedCustomDate}");
    ...
    if (widget.checkoutController.timeSlots != null &&
      ...
      DateTime date = widget.checkoutController.selectedDateSlot == 0
          ...
          : widget.checkoutController.selectedDateSlot == 1
              ? DateTime.now().add(const Duration(days: 1))
              : widget.checkoutController.selectedCustomDate ?? DateTime.now();
      DateTime endTime = widget.checkoutController
          .timeSlots![widget.checkoutController.selectedTimeSlot!].endTime!;
  ...
  bool _checkAvailability(
    ...
    if (widget.checkoutController.timeSlots == null ||
        widget.checkoutController.timeSlots!.isEmpty) {
      isAvailable = false;
    } else {
      for (CartModel cart in widget.cartList) {
        if (!DateConverter.isAvailable(
              ...
              time: widget.checkoutController.restaurant!.scheduleOrder!
                  ? scheduleStartDate
                  : null,
            ) &&
  ...
  bool _showsWarningMessage(BuildContext context, bool isGuestLogIn,
    ...
    if (isGuestLogIn &&
        ...
        widget.checkoutController.orderType != 'take_away') {
      showCustomSnackBar('please_setup_your_delivery_address_first'.tr);
      return true;
    } else if (isGuestLogIn &&
        widget.checkoutController.orderType == 'take_away' &&
        widget.guestNameTextEditingController.text.isEmpty) {
      showCustomSnackBar('please_enter_contact_person_name'.tr);
      return true;
    } else if (isGuestLogIn &&
        widget.checkoutController.orderType == 'take_away' &&
        widget.guestNumberTextEditingController.text.isEmpty) {
      ...
      return true;
        ...
        (widget.checkoutController.preferableTime.isEmpty ||
            widget.checkoutController.preferableTime == 'Not Available')) {
      ...
      showCustomSnackBar(
          '${'minimum_order_amount_is'.tr} ${widget.checkoutController.restaurant!.minimumOrder}');
      return true;
    } else if (widget.checkoutController.subscriptionOrder &&
        widget.checkoutController.subscriptionRange == null) {
      showCustomSnackBar('select_a_date_range_for_subscription'.tr);
      return true;
    } else if (widget.checkoutController.subscriptionOrder &&
        !datePicked &&
        widget.checkoutController.subscriptionType == 'daily') {
      ...
      return true;
        ...
        (widget.checkoutController.selectedDateSlot == 2 &&
            widget.checkoutController.customDateRestaurantClose)) {
      ...
      showCustomSnackBar(
          '${'you_cant_order_more_then'.tr} ${PriceConverter.convertPrice(widget.maxCodOrderAmount)} ${'in_cash_on_delivery'.tr}');
      return true;
    } else if (widget.checkoutController.timeSlots == null ||
        widget.checkoutController.timeSlots!.isEmpty) {
      if (widget.checkoutController.restaurant!.scheduleOrder! &&
  ...
  AddressModel? _processFinalAddress(bool isGuestLogIn) {
    ...
    AddressModel? finalAddress = isGuestLogIn
        ? widget.checkoutController.guestAddress
        : widget.checkoutController.address[widget.checkoutController.addressIndex];

    print("klklk ${jsonEncode(finalAddress)}");
    if (isGuestLogIn && widget.checkoutController.orderType == 'take_away') {
      String number = widget.checkoutController.countryDialCode! +
          widget.guestNumberTextEditingController.text;
      finalAddress = AddressModel(
        pincode: widget.checkoutController
            .address[widget.checkoutController.addressIndex].pincode!,
        cityName: widget.checkoutController
            .address[widget.checkoutController.addressIndex].cityName!,
        countryName: widget.checkoutController
            .address[widget.checkoutController.addressIndex].countryName!,
        streetName: widget.checkoutController
            .address[widget.checkoutController.addressIndex].streetName!,
        stateName: widget.checkoutController
            .address[widget.checkoutController.addressIndex].stateName!,
        contactPersonName: widget.guestNameTextEditingController.text,
        ...
        days.add(place_order_model.SubscriptionDays(
          day: widget.checkoutController.subscriptionType == 'weekly'
              ...
              : widget.checkoutController.subscriptionType == 'monthly'
                  ...
                  : index.toString(),
          time:
              DateConverter.dateToTime(widget.checkoutController.selectedDays[index]!),
        ));
      }
    }
    return days;
  ...
  PlaceOrderBodyModel _preparePlaceOrderModel(
      ...
      bool isGuestLogIn,
      List<place_order_model.SubscriptionDays> days) {
    log("llklk 65656 ${finalAddress!.pincode.toString()}");
    String pincode = finalAddress!.pincode.toString();
    return PlaceOrderBodyModel(
      pincode: finalAddress!.pincode.toString(),
      cityName: "${finalAddress.cityName.toString()}",
      countryName: "${finalAddress.countryName}",
      stateName: "${finalAddress.stateName}",
      streetName: "${finalAddress.streetName}",
      cart: carts,
      ...
      paymentMethod: widget.checkoutController.paymentMethodIndex == 0
          ? 'cash_on_delivery'
          : widget.checkoutController.paymentMethodIndex == 1
              ? 'wallet'
              : widget.checkoutController.paymentMethodIndex == 2
                  ? 'digital_payment'
                  : 'offline_payment',
      ...
      deliveryInstruction: widget.checkoutController.selectedInstruction != -1
          ? AppConstants
              .deliveryInstructionList[widget.checkoutController.selectedInstruction]
          : '',
