...
class RestaurantRepository implements RestaurantRepositoryInterface {
  ...
  Future<ProductModel?> getRestaurantProductList(int? restaurantID, int offset, int? categoryID, String type) async {
    ...
    Response response = await apiClient.getData(
      '${AppConstants.restaurantProductUri}?restaurant_id=$restaurantID&category_id=$categoryID&offset=$offset&limit=1000000000&type=$type',
    );
    if (response.statusCode == 200) {
  ...
  Future<ProductModel?> getRestaurantSearchProductList(String searchText, String? storeID, int offset, String type) async {
    ...
    Response response = await apiClient.getData(
      '${AppConstants.searchUri}products/search?restaurant_id=$storeID&name=$searchText&offset=$offset&limit=100000000&type=$type',
    );
    if (response.statusCode == 200) {
  ...
  @override
  Future<RestaurantModel?> getList({int? offset, String? filterBy, int? topRated, int? discount, int? veg, int? nonVeg,int? cuisine, bool fromMap = false, bool isSix = false}) async {
    RestaurantModel? restaurantModel;
    print("lklkljkl $nonVeg $veg ${cuisine}");
    Response response =
        cuisine.toString() !="0" ?
    await apiClient.getData('${AppConstants.restaurantUri}/all?offset=${isSix?1:offset}&limit=${isSix?6:fromMap ? 20 : 12}&filter_data=$filterBy&top_rated=$topRated&discount=$discount&veg=$veg&non_veg=$nonVeg&cuisine=$cuisine')
   :
    await apiClient.getData('${AppConstants.restaurantUri}/all?offset=${isSix?1:offset}&limit=${isSix?6:fromMap ? 20 : 12}&filter_data=$filterBy&top_rated=$topRated&discount=$discount&veg=$veg&non_veg=$nonVeg');

    if (response.statusCode == 200) {
      log("ddddd ${response.body}");
  ...
  Future<List<Restaurant>?> _getPopularRestaurantList(String type) async {
    ...
    if (response.statusCode == 200) {
      ...
      print("pp==pp${jsonEncode(popularRestaurantList)}");
    }
