...
class RestaurantController extends GetxController implements GetxService {
 ...
 int _cuisine = 0;
  ...
  Future<void> getRestaurantList(int offset, bool reload, {bool fromMap = false, bool isSix = false}) async {
    if(reload) {
      _restaurantModel = null;
      update();
    }
print("uuiuiuuu ${_cuisine}");
    RestaurantModel? restaurantModel = await restaurantServiceInterface.getRestaurantList(
        offset, _restaurantType, _topRated, _discount, _veg, _nonVeg,_cuisine, fromMap: fromMap,isSix: isSix);
  ...
  void setVeg() {
    print("Current _veg value: $_veg");

    // Toggle _veg between 0 and 1
    _veg = (_veg == 1) ? 0 : 1;

    // If _veg is 1, set _nonVeg to 0
    if (_veg == 1) {
      _nonVeg = 0;
    }

    // Update restaurant list or any other necessary action
    getRestaurantList(1, true);
  }
  ...
  void setNonVeg() {
    print("Current _nonVeg value: $_nonVeg");

    // Toggle _nonVeg between 0 and 1
    _nonVeg = (_nonVeg == 1) ? 0 : 1;

    // If _nonVeg is 1, set _veg to 0
    if (_nonVeg == 1) {
      _veg = 0;
    }
  ...
  void resetAll() {
    // Set both _veg and _nonVeg to 0
    _veg = 0;
    _nonVeg = 0;

    print("Both _veg and _nonVeg are reset to 0");
  ...
  void setPopular() {
    print("kkadj ${_popular}");
    _popular = restaurantServiceInterface.setPopular(_popular);
    getRestaurantList(1, true);
  }
  void setLatest() {
    _latest = restaurantServiceInterface.setLatest(_latest);
    getRestaurantList(1, true);
  }
  ...
  Future<void> getPopularRestaurantList(bool reload, String type, bool notify) async {
    ...
    if(_popularRestaurantList == null || reload) {
      ...
      print("======----${jsonEncode(_popularRestaurantList?[0])})");
      update();
    }
  }
  ...
  void categoryIndex1(int index) {
    _categoryIndex = index;
    update(); // if using GetX to update the UI when categoryIndex changes
  }
