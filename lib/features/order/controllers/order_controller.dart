...
class OrderController extends GetxController implements GetxService {
  ...
  Map<String, dynamic>?_thirdpartyCurrentstatusofOrderlist= {};
  Map<String, dynamic>? get thirdpartyCurrentstatusofOrderlist => _thirdpartyCurrentstatusofOrderlist;
  ...
  Future<List<OrderDetailsModel>?> getOrderDetails(String orderID,String addloguuid,) async {
    _isLoading = true;
    _showCancelled = false;
    _thirdpartyCurrentstatusofOrderlist=null;
    ...
    if (response.statusCode == 200) {
      ...
      if (addloguuid !="null"){
        await getOrderCurrentstatus(addloguuid);}
      log("kkkk22122 ${_thirdpartyCurrentstatusofOrderlist}");
    }
    _isLoading = false;
    update();
  ...
  Future getOrderCurrentstatus(String addloguuid) async {

    Response response = await orderServiceInterface.getOrderCurrentstatus(addloguuid);
    log("kkkk2212 ${response.body}");
    if (response.statusCode == 200) {
      _thirdpartyCurrentstatusofOrderlist=response.body;
      log("kkkk22122 ${_thirdpartyCurrentstatusofOrderlist}");
    }
    return _thirdpartyCurrentstatusofOrderlist;
  }
