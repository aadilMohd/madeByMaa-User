...
import 'package:http/http.dart'as re;
...
class LocationController extends GetxController implements GetxService {
  ...
  Future<AddressModel> getCurrentLocation(bool fromAddress,
    ...
    addressModel = AddressModel(
      ...
      cityName: getCityName(addressFromGeocode,3),
      stateName:  getCityName(addressFromGeocode,2),
      countryName:  getCityName(addressFromGeocode,1)
    );
    cityName = getCityName(addressFromGeocode,3);

    getAddressFromLatLng(LatLng(myPosition.latitude, myPosition.longitude));
    print("555555%%%%%%%%%%");
    print("555555%%%%%%%%%%$addressFromGeocode");
    print("666666%%%%%%%%%%$cityName");
    _loading = false;
    update();
    return addressModel;
  }

  String getCityName(String fullAddress,int index) {
    List<String> addressParts = fullAddress.split(', ');
    return addressParts.length >= 2 ? addressParts[addressParts.length - index] : '';
  }
  ...
  void updatePosition(CameraPosition? position, bool fromAddress) async {
    if (_updateAddressData) {
      ...
      if (_changeAddress) {
        ...
        cityName = getCityName(addressFromGeocode,3);
        getAddressFromLatLng(LatLng(position.target.latitude, position.target.longitude));
        print("555555%%%%%%%%%%$addressFromGeocode");
        print("666666%%%%%%%%%%$cityName");
        fromAddress
            ? _address = addressFromGeocode
