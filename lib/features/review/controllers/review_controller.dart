...
class ReviewController extends GetxController implements GetxService {
  ...
  List<FoodReviewedModel>? _foodReviewList;
  List<FoodReviewedModel>? get foodReviewList => _foodReviewList;
  ...
  Future<void> getReviewedProductList(bool reload, String type, bool notify) async {
    log("dataa off review ${reload}  ${type}  ${notify}");
    _reviewedType = type;
    if(reload) {
  ...
  Future<void> getFoodReviewedList(String? foodID) async {
    _foodReviewList = await reviewServiceInterface.getFoodReviewedListt(foodID);
