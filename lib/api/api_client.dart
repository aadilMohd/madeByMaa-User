...
class ApiClient extends GetxService {
  ...
  Map<String, String> updateHeader(String? token, List<int>? zoneIDs,
    ...
    header.addAll({
      ...
      AppConstants.zoneId: (zoneIDs != null && zoneIDs.isNotEmpty)
          ? jsonEncode([zoneIDs[0]])
          : '',
  ...
  Future<Response> getData(String uri,
    ...
    try {
      if (kDebugMode) {
        log('====> API Call: $uri\nHeader: $_mainHeaders');
        debugPrint('====> API Call1: $uri\nHeader: $_mainHeaders');
        log("kjjjjj ${_mainHeaders}");
      }
  ...
  // Future<Response> getDataThirdParty(String uri, {Map<String, dynamic>? query, Map<String, String>? headers, bool handleError = true, bool showToaster = false}) async {
  //   try {
  //     if(kDebugMode) {
  //       log('====> API Call: $uri\nHeader: $_mainHeaders');
  //       debugPrint('====> API Call1: $uri\nHeader: $_mainHeaders');
  ...
  //       Uri.parse(AppConstants.thirdpartydevbaseUrl+uri),
  //       headers: headers ,
  //     ).timeout(Duration(seconds: timeoutInSeconds));
  ...
  Future<Response> getDataThirdParty(String uri, dynamic body,
      {Map<String, String>? headers, bool handleError = true}) async {
    try {
      if (kDebugMode) {
        log('====> API Call: ${AppConstants.thirdpartydevbaseUrl + uri}\nHeader: $headers');
        log('====> API Body222: ${jsonEncode(body)}');
      }
      http.Response response = await http
          .post(
            Uri.parse(AppConstants.thirdpartydevbaseUrl + uri),
